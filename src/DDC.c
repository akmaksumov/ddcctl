...
bool DDCWrite(io_service_t framebuffer, struct DDCWriteCommand *write)
{
    ...
    data[0] = write->i2c_source_address;
    data[1] = 0x84;
    data[2] = 0x03;
...
bool DDCRead(io_service_t framebuffer, struct DDCReadCommand *read)
{
    ...
    for (int i = 1; i <= kMaxRequests; i++)
    {
        ...
        // data[0] = 0x51;
        data[0] = read->i2c_source_address;
        data[1] = 0x82;
        data[2] = 0x01;
        ...
        // request.replySubAddress = read->i2c_source_address;
    ...
    while ((io_service = IOIteratorNext(io_objects)) != MACH_PORT_NULL)
    {
        ...
        if (kr == KERN_SUCCESS)
        {
            ...
            if (types)
            {
                ...
                printf("E: Fatal - No supported Transaction Types! \n");
