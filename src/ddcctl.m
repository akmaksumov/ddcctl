//
//  ddcctl.m
//  query and control monitors through their on-wire data channels and OSD
//  microcontrollers http://en.wikipedia.org/wiki/Display_Data_Channel#DDC.2FCI
...
//  Added optional use of an external app 'OSDisplay' to have a BezelUI like
//  OSD. Have fun! Marc (Saman-VDR) 2016
...
NSString *getDisplayDeviceLocation(CGDirectDisplayID cdisplay) {
  ...
  // TODO: try shelling `system_profiler SPDisplaysDataType -xml` to get
  // "_spdisplays_displayPath" keys
  ...
  if (!wsDisplaySets) {
    MyLog(@"E: Failed to get 'DisplayAnyUserSets' key from WindowServer's "
          @"preferences! (%@)",
          wsPrefs);
    return NULL;
  }

  // $ PlistBuddy -c "Print DisplayAnyUserSets:0:0:IODisplayLocation" -c "Print
  // DisplayAnyUserSets:0:0:DisplayID"
  ...
  for (NSArray *displaySet in wsDisplaySets) {
    for (NSDictionary *display in displaySet) {
      if ([[display valueForKey:@"DisplayID"] integerValue] == cdisplay) {
        return
            [display valueForKey:@"IODisplayLocation"]; // kIODisplayLocationKey
      }
    }
  }

  MyLog(@"E: Failed to find display in WindowServer's preferences! (%@)",
        wsPrefs);
  return NULL;
}

/* Get current value for control from display */
uint getControlV2(CGDirectDisplayID cdisplay, uint control_id, uint i2c_source_address) {
  struct DDCReadCommand command;
  command.control_id = control_id;
  command.max_value = 0;
  command.current_value = 0;
  command.i2c_source_address = i2c_source_address;
  MyLog(@"D: querying VCP control: #%u =?, i2c-address-source: 0x%02hhx", 
        command.control_id, command.i2c_source_address);

  if (!DDCRead(cdisplay, &command)) {
    MyLog(@"E: DDC send command failed!");
    MyLog(@"E: VCP control #%u (0x%02hhx) = current: %u, max: %u",
          command.control_id, command.control_id, command.current_value,
          command.max_value);
  } else {
    MyLog(@"I: VCP control #%u (0x%02hhx) = current: %u, max: %u",
          command.control_id, command.control_id, command.current_value,
          command.max_value);
  }
...
uint getControl(CGDirectDisplayID cdisplay, uint control_id) {
  return getControlV2(cdisplay, control_id, 0x51);
}
/* Set new value for control from display */
void setControlV2(io_service_t framebuffer, uint control_id, uint new_value,
                  uint i2c_source_address) {
  struct DDCWriteCommand command;
  command.control_id = control_id;
  command.new_value = new_value;
  command.i2c_source_address = i2c_source_address;
  // MyLog(@"D: setting VCP control #%u => %u", command.control_id,
  // command.new_value);
  MyLog(@"D: setting VCP control #%u => %u, i2c-address-source: 0x%02hhx",
        command.control_id, command.new_value, command.i2c_source_address);
  ...
  if (useOsd) {
    NSString *OSDisplay =
        @"/Applications/OSDisplay.app/Contents/MacOS/OSDisplay";
    switch (control_id) {
    case 16:
      [NSTask
          launchedTaskWithLaunchPath:OSDisplay
                           arguments:[NSArray
                                         arrayWithObjects:
                                             @"-l",
                                             [NSString
...
void setControl(io_service_t framebuffer, uint control_id, uint new_value) {
  setControlV2(framebuffer, control_id, new_value, 0x51);
}

/* Get current value to Set relative value for control from display */
void getSetControl(io_service_t framebuffer, uint control_id,
                   NSString *new_value, NSString *operator) {
  struct DDCReadCommand command;
  command.control_id = control_id;
  command.max_value = 0;
  command.current_value = 0;
  command.i2c_source_address = 0x51;
  ...
  // calculate
    ...
    MyLog(@"D: relative setting: %@ = %d (clamped to 0, %d)", formula,
          clamped_value, command.max_value);
...
int main(int argc, const char *argv[]) {
  ...
  @autoreleasepool {
    ...
    for (NSScreen *screen in NSScreen.screens) {
      ...
      if ([description objectForKey:@"NSDeviceIsScreen"]) {
        ...
        if (CGDisplayIsBuiltin(screenNumber))
          continue; // ignore MacBook screens because the lid can be closed and
                    // they don't use DDC.
        ...
        [_displayIDs
            addObject:[NSNumber numberWithUnsignedInteger:screenNumber]];
        NSSize displayPixelSize =
            [[description objectForKey:NSDeviceSize] sizeValue];
        CGSize displayPhysicalSize = CGDisplayScreenSize(
            screenNumber); // dspPhySz only valid if EDID present!
        ...
        if (displayScale > 1) {
          MyLog(@"D: CGDisplay %@ dispID(#%u) (%.0fx%.0f %g°) HiDPI",
                screenUUIDstr, screenNumber, displayPixelSize.width,
                displayPixelSize.height, rotation);
        } else {
    ...
    MyLog(@"I: found %lu external display%@", [_displayIDs count],
          [_displayIDs count] > 1 ? @"s" : @"");
    ...
    BOOL dump_values = NO;
    uint i2c_source_address = 0x51; // default/original channel id
    NSString *HelpString =
        @"Usage:\n"
        @"ddcctl \t-d <1-..>  [display#]\n"
        ...
        @"\n"
        @"----- Settings that use non-standard channel (some LG models) -----\n"
        @"\t-i_lg <144-210>  [select input source]\n"
        @"\n"
        @"----- Settings (testing) -----\n"
        @"\t-rg <1-..>  [red gain]\n"
        @"\t-gg <1-..>  [green gain]\n"
    ...
    for (int i = 1; i < argc; i++) {
      if (!strcmp(argv[i], "-d")) {
        ...
        if (i >= argc)
          break;
        [actions setObject:@[
          @BRIGHTNESS, [[NSString alloc] initWithUTF8String:argv[i]]
        ]
                    forKey:@"b"];
      }
        ...
        if (i >= argc)
          break;
        [actions setObject:@[
          @CONTRAST, [[NSString alloc] initWithUTF8String:argv[i]]
        ]
                    forKey:@"c"];
      }

      else if (!strcmp(argv[i], "-rbc")) {
        [actions setObject:@[ @RESET_BRIGHTNESS_AND_CONTRAST, @"1" ]
                    forKey:@"rbc"];
      }
        ...
        if (i >= argc)
          break;
        [actions setObject:@[
          @RED_GAIN, [[NSString alloc] initWithUTF8String:argv[i]]
        ]
                    forKey:@"rg"];
      }
        ...
        if (i >= argc)
          break;
        [actions setObject:@[
          @GREEN_GAIN, [[NSString alloc] initWithUTF8String:argv[i]]
        ]
                    forKey:@"gg"];
      }
        ...
        if (i >= argc)
          break;
        [actions setObject:@[
          @BLUE_GAIN, [[NSString alloc] initWithUTF8String:argv[i]]
        ]
                    forKey:@"bg"];
      }
        ...
        if (i >= argc)
          break;
        /* was DPMS toggle, but now is no-op because it was bricking monitors
      ...
      else if (!strcmp(argv[i], "-osd")) { // read only - returns '1' (OSD
                                           // closed) or '2' (OSD active)
        ...
        if (i >= argc)
          break;
        [actions setObject:@[
          @COLOR_PRESET_A, [[NSString alloc] initWithUTF8String:argv[i]]
        ]
                    forKey:@"preset_a"];
      }
        ...
        if (i >= argc)
          break;
        [actions setObject:@[
          @COLOR_PRESET_B, [[NSString alloc] initWithUTF8String:argv[i]]
        ]
                    forKey:@"preset_b"];
      }
        ...
        if (i >= argc)
          break;
        [actions setObject:@[
          @COLOR_PRESET_C, [[NSString alloc] initWithUTF8String:argv[i]]
        ]
                    forKey:@"preset_c"];
      }
        ...
        if (i >= argc)
          break;
        [actions setObject:@[
          @INPUT_SOURCE, [[NSString alloc] initWithUTF8String:argv[i]]
        ]
                    forKey:@"i"];
      }else if (!strcmp(argv[i], "-i_lg")) { // alternate input switch for some LG monitors
        i++;
        if (i >= argc)
          break;
        [actions setObject:@[
          @INPUT_SOURCE_LG, [[NSString alloc] initWithUTF8String:argv[i]]
        ]
                    forKey:@"i_lg"];
        i2c_source_address = 0x50; // alternate channel id for some LG monitors
      } else if (!strcmp(argv[i], "-m")) {
        i++;
        if (i >= argc)
          break;
        [actions setObject:@[
          @AUDIO_MUTE, [[NSString alloc] initWithUTF8String:argv[i]]
        ]
                    forKey:@"m"];
      }
        ...
        if (i >= argc)
          break;
        [actions setObject:@[
          @AUDIO_SPEAKER_VOLUME, [[NSString alloc] initWithUTF8String:argv[i]]
        ]
                    forKey:@"v"];
      }
        ...
        if (i >= argc)
          break;
        [actions
            setObject:@[ test, [[NSString alloc] initWithUTF8String:argv[i]] ]
               forKey:@"test"];
        NSLog(@"TEST: %@  %@", test,
              [[NSString alloc] initWithUTF8String:argv[i]]);
      }
#endif
    ...
    CGDirectDisplayID cdisplay =
        ((NSNumber *)_displayIDs[displayId - 1]).unsignedIntegerValue;

    // find & grab the IOFramebuffer for the display, the IOFB is where DDC/I2C
    // commands are sent
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    if (CGDisplayIOServicePort != NULL) {
      // legacy API call to get the IOFB's service port, was deprecated after
      // macOS 10.9:
...
#pragma clang diagnostic pop
    ...
    if (!framebuffer && devLoc) {
      // a devLoc is required because, without that IOReg path, this func is
      // prone to always match the 1st device of a monitor-pair (#17)
      framebuffer = IOFramebufferPortFromCGDisplayID(
          cdisplay, (__bridge CFStringRef)devLoc);
    }
    ...
    MyLog(@"I: polling EDID for #%lu (ID %u => %@)", displayId, cdisplay,
          devLoc);

    struct EDID edid = {};
    if (EDIDTest(framebuffer, &edid)) {
      for (union descriptor *des = edid.descriptors;
           des < edid.descriptors +
                     sizeof(edid.descriptors) / sizeof(edid.descriptors[0]);
           des++) {
    ...
    [actions enumerateKeysAndObjectsUsingBlock:^(
                 id argname, NSArray *valueArray, BOOL *stop) {
      ...
      if (control_id > -1) {
        ...
        NSString *argval_num = [argval
            stringByTrimmingCharactersInSet:
                [NSCharacterSet characterSetWithCharactersInString:
                                    @"-+"]]; // look for relative setting ops
        if ([argval hasPrefix:@"+"] ||
            [argval hasPrefix:@"-"]) { // +/-NN relative
          getSetControl(framebuffer, control_id, argval_num,
                        [argval substringToIndex:1]);
        } else if ([argval hasSuffix:@"+"] ||
                   [argval hasSuffix:@"-"]) { // NN+/- relative
          // read, calculate, then write
          getSetControl(framebuffer, control_id, argval_num,
                        [argval substringFromIndex:argval.length - 1]);
        } else if ([argval hasPrefix:@"?"]) {
          // read current setting
          getControlV2(framebuffer, control_id, i2c_source_address);
        } else if (argval_num == argval) {
          // write fixed setting
          setControlV2(framebuffer, control_id, [argval intValue],
                       i2c_source_address);
        }
      }
